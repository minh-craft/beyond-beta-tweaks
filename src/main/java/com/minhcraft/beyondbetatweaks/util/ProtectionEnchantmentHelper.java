...
public class ProtectionEnchantmentHelper {
    ...
    public static float getFloatDamageProtection(Iterable<ItemStack> stacks, DamageSource source) {
        ...
        EnchantmentHelper.runIterationOnInventory(
                (enchantment, i) -> {
                    if (enchantment instanceof IFractionalProtectionEnchantment) {
                        mutableFloat.add(((IFractionalProtectionEnchantment)enchantment).beyond_beta_tweaks$getFloatDamageProtection(i, source));
                    } else {
                        mutableFloat.add((enchantment).getDamageProtection(i, source));
                    }
                }, stacks);
